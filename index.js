...
let processorData = new Array(); // 각 프로세서 별 실행중인 프로세스
...
function rr(){ 
    ...
    while(checkRun == 1){ // run을 클릭했다면 && 모든 프로세스가 종료될때 까지 -> 시간시작
            ...
            for(let i =0; i<numberOfProcessor;i++){ // 모든 프로세서를 검사      
                for(let j = 0; j< runningProcess.length; j++){
                    if(runningProcess[j] != -1){
                        exitByQuantum = (Number(processData[runningProcess[j][0]][3]) + Number(quantumTime));
                        
                        processData[runningProcess[j][0]][7] = runningProcess[j][2]; // 퀀텀시간이 지나 프로세스가 레디큐로 넘어갈떄, 잔여시간을 업데이트
                        if(runningProcess[j][1] == i) {
                            if((runningProcess[j][2] != 0)&&(presentTime == exitByQuantum)&& (processorState[i] == 1)){ //퀀텀시간이 자났을 때(현재시간 = 시작시간+퀀텀시간)
                                processorData[runningProcess[j][1]] = -1; // 종료된 프로세스는 -1로 표시
                                exitCount++;
                                processorState[i] = -1; // 프로세서를 종료한다.
                                exitQuantumQueue.enqueue(processData[runningProcess[j][0]]); // 퀀텀시간이 자나 레디큐로 이동
                                runningProcess[j] = -1;
                                break;
                            }
        ...
        if(exitProcessQueue.toLength() >= numberOfProcess) break;
        
        
        
        for(let i=0;i<numberOfProcess; i++){ // 현재시간 == 도착시간인 프로세스가 있으면 레디큐에 삽입
            if (presentTime == processData[i][1]){
                readyQueue.enqueue(processData[i]);
            }
        }
        
        if(!exitQuantumQueue.empty()){
            temp = exitQuantumQueue.toLength();
            for(let i =0; i<temp;i++){
                readyQueue.enqueue(exitQuantumQueue.dequeue()); // 퀀텀에 의해 종료된 큐를 이후에 레디큐에 삽입
            }
        }
       showProcessData();
        ...
        while(readyQueue.empty() == false && processorState.indexOf(-1) >= 0){ 
            ...
            if(!dequeProcess[7]||!dequeProcess[3]){ // 디큐 프로세스의 잔여시간이 없거나 시작시간이 없으면
                dequeProcess[7] = dequeProcess[2]; // 디큐된 프로세스의 초기 잔여시간을 실행시간으로 설정함.
                processData[dequeProcess[0]-1][3] = presentTime; // 프로세스의 시작시간을 현재시간으로 설정
            }
            runningProcess.push([dequeProcess[0]-1,workIndex,dequeProcess[7]]); // 실행중인프로세스 목록에 [실행중인 프로세스, 어느 프로세서에서 실행중인지, 잔여시간(실행시간)] 부여
